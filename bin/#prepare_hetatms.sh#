...
if [ -z ${cube_dist} ]; then
    echo "${SCRIPT_NAME}: WARNING: cube_dist variable is empty. defaulting to 15"
    cube_dist=15
fi
...
for SDF_FILE in ${prediction_dir}/*.sdf;
do
    sed -i 's/-/ -/g' ${SDF_FILE}
done
...
#echo "DEBUGG DEBUGG: ${prediction_dir}"
# hierarchical clustering to get 3 similar but distinct sdfs (highest ranking predictions)
python $parent_dir/cluster_preds_com.py $prediction_dir $n_clust
#can use cluster_preds_com or new_cluster_preds.py for above 
